<?php
	...
	$sql = ("SELECT * FROM category, maincategory WHERE category.id_mainCategory = maincategory.id_mainCategory and maincategory.name_mainCategory = 'Широкоформатная печать'");
	$query = $pdo->prepare($sql);
	$query->execute();
	dbCheckError($query);
	$largeFormat = $query->fetchAll();
?>
    ...
    <body>
        <div class="wrapper">
            ...
            <main class="pageCategory">
				<div class="mt9"></div>
				<?php foreach ($largeFormat as $key => $category): ?>
						...
						<div class="product__column1">
							<article class="product__item1 item-product1">
								...
								<div class="item-product__image _ibg">
									<img src="assets/img/largeFormat/<?=$category['image_category'];?>" alt="<?=$category['name_category'];?>">
								</div>
							</article>
						</div>
					</div>
